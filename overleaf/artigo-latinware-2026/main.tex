...
\begin{tikzpicture}[
  ...
  bloco/.style   = {draw, rounded corners=1.5pt, align=center, inner sep=2.5pt,
                    text width=50mm, minimum height=7mm},
  obj/.style     = {draw, rounded corners=1.5pt, align=center, inner sep=2.5pt,
                    text width=22mm, minimum height=6.5mm, fill=black!7},
  medida/.style  = {draw, dashed, rounded corners=1.5pt, align=center,
                    inner sep=3pt, text width=72mm},
  seta/.style      = {-{Latex[length=1.5mm]}, thick},
  tracejada/.style = {-{Latex[length=1.5mm]}, dashed}
]

\node[bloco] (pop) {População: 300 indivíduos\\
  1 indivíduo $=$ 5 personagens $\times$ 10 genes};

\node[bloco, below=3.5mm of pop] (sim) {Simulação: \emph{round-robin}\\
  10 confrontos $\times$ 150 lutas};

\node[obj, below=3.5mm of sim, xshift=-16mm] (drift)
  {\emph{drift}\\[-1pt] identidade};
\node[obj, right=5mm of drift] (dom)
  {\emph{dominance}\\[-1pt] equilíbrio};

\node[bloco, below=14mm of sim] (oper)
  {seleção $\cdot$ \emph{crossover} $\cdot$ mutação $\cdot$ elitismo};

\node[medida, below=6mm of oper] (ph)
  {\textbf{Medição \emph{post hoc}} --- não realimenta a busca\\[1pt]
   validador (\valTotal{} asserções) $\cdot$ diferenciação $\cdot$
   perfil comportamental $\cdot$ ciclo de vantagens};
...
% laço de otimização: fecha pela esquerda
\draw[seta] (oper.west) -- ++(-6mm,0) |- (pop.west);

% ramo de medição: sai das mesmas simulações e NÃO retorna
\draw[tracejada] (sim.east) -| ($(ph.north east)+(-6mm,0)$);

\end{tikzpicture}
\caption{Arquitetura do sistema. O laço de otimização (setas cheias) fecha sobre os
dois objetivos; os instrumentos de identidade (tracejado) consomem as mesmas
simulações mas não retornam à busca --- a separação que sustenta a não
circularidade.}
\label{fig:arq}
\end{figure}
