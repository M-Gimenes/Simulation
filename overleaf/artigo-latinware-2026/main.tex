...
As contribuições são: (i) uma formulação do balanceamento como problema
\emph{bi-objetivo} explícito --- equilíbrio \emph{versus} identidade ---, otimizado
por um AG escalar e pelo NSGA-II \cite{deb2002nsga}; e (ii) instrumentos de medição
\emph{post hoc} que quantificam a identidade sem contaminar a função objetivo. Os
resultados são preliminares: a calibração dos limites dos genes e das bandas de
equilíbrio segue em refinamento.

\section{Metodologia}

A unidade de evolução é o \emph{conjunto dos cinco personagens}, pois a taxa de
vitória de qualquer um deles depende simultaneamente dos outros quatro. O sistema
tem duas camadas: simulação (combate 1\,vs\,1, tick a tick) e evolução.
...
$\mathrm{dano}\in[15,30]$ --- para eliminar modos de exploração degenerados.
...
pesos agirem de forma contínua, o que dá ao algoritmo um gradiente sobre esses
genes --- sob comparação categórica, só a ordem dos pesos importaria, e as
magnitudes seriam invisíveis à seleção.

A resolução de dano é determinística e \emph{flat}: o dano é o próprio atributo do
atacante, e o único modificador é a ação DEFEND do alvo, que o reduz em 40\%. O
\emph{stun} aplicado é uma fração $\in[0;\,0{,}6]$ do \emph{cooldown} do próprio
atacante; por ser inferior a 1, garante sempre uma janela livre entre golpes. A luta
...
com $\lambda_{d}=\lambda_{D}=1{,}0$; entrega \emph{um} ponto do compromisso. O
...
independentes, hipervolume da fronteira com referência fixa em $(2{,}0;\,1{,}0)$ ---
os piores valores possíveis dos dois objetivos --- e \emph{validação externa} ao
\emph{fitness}, ao estilo Ludi \cite{browne2010evolutionary}, que reavalia um
indivíduo fixo sob sementes inteiramente novas. As execuções individuais usam a
semente \seedMain{}.
...
\blowCan{} dos dez confrontos sendo \emph{blowouts}. Esse resultado é, em si, um
achado: o ciclo de vantagens canônico não é trivialmente preservado em um modelo
de combate determinístico --- apenas \cycleCan{} dos dez pares do ciclo se
sustentam ---, o que sugere que a estrutura não transitiva dos jogos de luta reais
depende parcialmente de mecânicas deliberadamente ausentes do modelo.
...
$\hvVal{}$. Ela evidencia que equilíbrio e identidade \emph{são} objetivos
conflitantes neste domínio e quantifica \emph{quanto} de um se troca pelo outro.
...
$\textit{dominance}=\aggDomMean{}\pm\aggDomStd{}$,
$\textit{drift}=\aggDriftMean{}\pm\aggDriftStd{}$ e hipervolume
$\aggHvMean{}\pm\aggHvStd{}$. O equilíbrio \emph{global} é a parte estrutural ---
cada personagem permanece equilibrado em 90--100\% das execuções ---, ao passo que
zerar todos os \emph{hard-counters} de par é sensível à semente
($\hcPerSeed{}\pm\hcPerSeedStd{}$ pares residuais por execução). Fora do laço de
otimização, sob \extNSeeds{} sementes novas, os \robCharsBd{} personagens seguem
globalmente equilibrados e \robBd{} dos \robTotal{} confrontos permanecem
equilibrados em \emph{todas} as condições: o equilíbrio reportado \textbf{não é}
...
\emph{roster} atingem equilíbrio. O trabalho se insere na tradição de ajuste de
conteúdo de jogos por busca \cite{togelius2011searchbased, hom2007automatic}.
