...
Uso:
    py -m src.visualization.viewer                        # matchup aleatório (canônico)
    py -m src.visualization.viewer rushdown grappler      # matchup específico
    py -m src.visualization.viewer rushdown grappler --delay 0.04
    py -m src.visualization.viewer --list                 # lista arquétipos disponíveis
    py -m src.visualization.viewer --evolved              # usa resultado da última execução do AG
    py -m src.visualization.viewer --nsga2                # usa knee_point do NSGA-II
    py -m src.visualization.viewer --nsga2 best_dominance # usa representante específico do NSGA-II
    py -m src.visualization.viewer --all                  # roda todos os 10 matchups em sequência
    py -m src.visualization.viewer --no-vs                # pula a tela de apresentação
...
def main() -> None:
    ...
    parser.add_argument("--evolved", action="store_true",
                        help="Usa personagens do último AG (single_run/ga.json)")
