...
The intention sampling is the **only** stochastic node in the loop. Weights act continuously: a Δ in any weight produces a proportional Δ in intention probability, giving the GA a continuous gradient on these genes. Because sampling is proportional, only the **ratio** between the three weights affects behaviour — which is why `drift_genes` rescales the three to the canonical sum before the drift term compares them, instead of charging identity for a behaviourally null degree of freedom.
...
- `dominance_penalty` (via `LAMBDA_DOMINANCE=1.0`, **C2 formulation**) = `DOMINANCE_GLOBAL_WEIGHT·global_term + DOMINANCE_CAP_WEIGHT·cap_term + DOMINANCE_DECIS_WEIGHT·decis_term` (weights 1.0 / 0.5 / 0.5; max 2.0). **Primary `global_term`** = RMS over the 5 characters of `|WR_global − 0.5| / 0.5` — *no archetype globally dominates the roster*. It does **not** force every pair to 50%: a character at 50% global can beat 2 and lose 2 — the space where the advantage cycle can live. (The old primary was per-matchup WR, whose optimum is *every pair at 50%* = flat balance, **incompatible with a cycle by construction** — see `docs/reference/11-combat-review.md` and `docs/tcc/02-ciclo-canonico.md`.) **Secondary `cap_term`** = RMS over the 10 pairs of the excess of `|WR_par − 0.5|` above `MATCHUP_WR_CAP` (hard-counter ceiling: cycle edges stay as advantages within `[0.35, 0.65]`, crushing counters like 100×0 are barred). **Secondary `decis_term`** = RMS over the 10 pairs of per-fight decisiveness `D = mean(|score − 0.5|)` outside the band `[MATCHUP_FLOOR, MATCHUP_THRESHOLD] = [0.02, 0.20]`; per-fight score is continuous (KO: `0.5 + 0.5·winner_HP_frac`; timeout: HP-share). The ceiling guards against blowout-coinflip (55%/45% crush each way, global WR ~50% but every fight a blowout); the floor only guards against degeneracy. The graded WR that makes this a usable gradient comes from the intention-sampling noise once fights are close. The three terms are kept **separate** in `DominanceTerms` and reported decomposed by `multi_run` / `compare_algorithms` — the composite alone hides whether a difference came from the primary term or from a secondary one at half the weight. **The secondary terms are load-bearing, and that is now measured rather than argued** (sweep of 2026-09-17, 5 arms × 5 seeds at reduced budget): with both secondaries at zero the GA reaches the **best `global_term` of every arm** — 0.0170, since it is the only thing left to optimise — and still delivers **10 of 10 pairs as hard counters, in 5 of 5 seeds**. That is precisely the blowout-coinflip this formulation predicted, with the strongest possible signature (10/10, zero deviation). And `decis_term`, which reads 0.0000 on every healthy final individual and had been suspected of being dead, is **not inert**: removing it alone triples hard counters (0.6 → 3.2) and *worsens `cap_term` itself* (0.0063 → 0.1440) — the two guards are complementary, since without the decisiveness floor the search makes fights decisive and decisive fights push pairs out of the WR band the cap protects. Comparing arms by the composite would be an error — the weights *define* it; the comparison is by the terms and the post-hoc metrics, which is what the decomposition is for. The 0.5/0.5 split is kept: raising the cap weight improves counters and convergence at a drift cost, but at n = 5 the difference (0.6 ± 0.5 vs 0.2 ± 0.4) is indistinguishable from noise.
...
**Every post-hoc metric is read against a measured floor, never against the ceiling.** None of the project's identity metrics has a floor of zero, and all three were being read as if they did. Measured (`src/tools/baselines.py`, 35 null rosters — 5 mirrors + 30 random, the current battery; 30 is the tool's default, `N_RANDOM_DEFAULT`, because the battery calls it without a flag and the p-value's resolution is 1/N): the **validator**'s chance floor is 6.4/23 and the best null reached **10/23**, because rank assertions resolve ties by index and some pass by accident; **drift** reads 0.377 for a mirror (five identical characters — identity zero by construction) and 0.415 for a random roster, so only ~0.04 separates "identity carefully preserved" from total annihilation; the **canonical cycle** floor is 5/10, since every edge is a coin flip. Reading a raw validator score as "that fraction of identity survived" is the same error as reading 20% on a five-option multiple-choice test as "knows 20% of the material". So every metric is reported as `position = (value − floor) / (ceiling − floor)` plus an **empirical p-value** (what fraction of null rosters match or beat it) — the floor is a *distribution*, not a point.

Two consequences worth stating plainly. First, the **mirror roster is the trivial solution** to balance (perfect balance, zero identity) and it is the numeric answer to the obvious objection "why not just make all five identical?" — measured on the current battery, the evolved roster reaches **102%** of mirror-level balance (it is *more* balanced than five identical characters) while sitting at **47%** of the way from the identity floor to the ceiling, beating all 35 nulls on every identity axis (p < 0.03 each). Second, **the authored cycle cannot be a result**: it is a *regular tournament* (each archetype beats exactly 2), and there are **24** labelled regular tournaments on 5 vertices, so hitting the specific one is a 1-in-24 lottery while the chance score is 5/10. What *is* a result, and needs no authorship, is **intransitivity itself** — global WR balance and rock-paper-scissors are the same structure, since a strictly transitive roster has WRs 100/75/50/25/0, incompatible with everyone near 50%. `circular_triads` measures it on a 0 (strict pecking order) / 2.5 (random) / 5 (maximum = perfectly balanced) scale, and is only meaningful when the pair WRs are decided rather than noise.
...
**Every artifact carries the configuration that produced it, and checks itself on load.** `src/engine/provenance.py` stamps every JSON in `results/` with a `provenance` block: timestamp, a `fingerprint`, **every** public constant of `config.py` value by value, a digest of the canonical archetypes (genes + `defining_genes` + `beats`) and a digest of `src/engine/`'s source. `Individual.from_results` / `from_nsga2` call `warn_if_stale` — those two constructors are the chokepoint every tool goes through to load an evolved individual, so a new tool cannot be born without the check. The warning names *what* changed (`MATCHUP_WR_CAP: 0.20 → 0.15`), not just that something did. Four choices, each against a specific failure: constants are **enumerated** from `config.py` rather than hand-listed (a curated list rots — the next constant added would be invisible to the stamp); the engine's **source** is hashed, not only its constants (the incident that motivated this was code — `grab_power`, collision, stream rotation — and a constants-only stamp would have read "current"); `config.py` is excluded from the source digest because its values are recorded one by one, which is actionable where a hash is not; and `N_WORKERS` is excluded because evaluation reseeds to `_SEED_BASE` before every round-robin, so worker count changes no number — an alarm that cries wolf stops being read. `MULTI_RUN_N_SEEDS` (20, the protocol value) is excluded for the same reason: it only sets the `multi_run` sample size, which that artifact records in its body; `compare` ignores excluded constants on the recorded side too, so artifacts stamped before the exclusion stay current. The 2026-09-18 battery is the first generated entirely under the module. The retroactive stamp the earlier artifacts carried had been justified by reproducing two of them and *inferring* the rest, and the inference was wrong for one — the sensitivity artifact was from 2026-09-16; so a retroactive stamp is only valid by reproducing **that** artifact. Note for anyone reproducing an artifact: the recorded number is measured on the **last generation's** stream (`generation_seed(seed, MAX_GENERATIONS)`), not on the seed base — reproducing the value means reproducing the stream.

**The NSGA-II front is λ-independent, which is what lets the λ sweep and the n=20 battery be one experiment.** `nsga2.scalar_objective` is the only consumer of `LAMBDA_*` in the algorithm, and it is *reporting, not search* — it picks which front point is called `scalar_optimum`; Pareto dominance and crowding never look at the weights. So **one** NSGA-II run serves every arm of the sweep: for each λ you re-derive that minimum from the already-saved front, no re-run. Together with the fact that the λ=1.0 / 20-seed cell **is** the main battery, the two experiments share everything expensive — run separately, the sweep would have to re-run NSGA-II (+4h42) or compare against a front of different provenance. To make the sweep possible the `LAMBDA_*` stopped being import-time constants and became **process state** (`fitness.set_lambdas` / `get_lambdas`), for the same reason and with the same care as `_SEED_BASE`: on Windows the pool spawns and re-imports the module, so without explicit propagation through `_init_worker` the workers would evaluate at `config.py`'s λ while the parent uses the arm's — an entire arm measuring the wrong λ with no symptom, surfacing as a result rather than an error. Covered by a test that compares the parallel path against the serial one under two λ. The runtime override registers itself in the provenance stamp (`provenance.override`), so an arm's artifact records the λ it actually used and gets its own fingerprint; `Divergence.is_experiment_arm` then separates "arm of an experiment" from "stale artifact" — strictly, since any divergence outside the declared overrides makes it stale again, otherwise the override would be a free pass for hiding an engine change. The sweep itself was then run cheaply instead — 5 arms at pop 120 x 60 generations, 25 minutes, confirming λ=1.0 as the curve's knee without changing config. `run_battery.ps1` ran the n=20 battery (2026-09-18, 11 resumable steps, 5h54; `-From N`, `-WhatIf` to just cost it), and it saves `front_objectives` per seed precisely so a future λ comparison against the front needs no NSGA-II re-run. Seeds 42–51 reproduced the n = 10 battery bit for bit. All three Holm metrics stay significant with a large effect — `dominance` p 0.0123 (Â₁₂ 0.27, GA better), `drift` p 0.00007 (0.89, NSGA-II better), hard counters p 0.0018 (0.21, GA better) — and all three Â₁₂ moved toward 0.5 from n = 10 (0.20 / 0.94 / 0.14): the small-sample effect was inflated, and n = 20 is the one to cite.

**Elitism 10% and tournament 3 are tested values, not textbook defaults.** Swept on 2026-09-18 (7 arms × 5 seeds at the reduced budget: elitism 0 / 5% / 20% / 30%, tournament 2 / 5 / 7), no arm beats the default, which has the fewest hard counters (0.6 vs 1.0–2.2) and the lowest `cap_term` of the eight; the alternatives trade counters for a little drift. At n = 5 none of it separates from noise, and tournament size gives a non-monotonic pattern (3 better than both 2 and 5) that looks like noise rather than an optimum — so the claim is "tested, nothing beats them", not "optimal". Both are read only by `operators.py` in the parent process, so unlike λ and the dominance weights they need no `RuntimeState` propagation. With this, λ, the dominance weights and selection have all been swept against their current values, and all three held.

**`converged_at` / `stagnated_at` are recorded per seed, and the asymmetry is declared.** `multi_run` writes both into `per_seed` and aggregates them into `convergence` (rate + mean generation among the seeds that converged) — this is the **speed** axis that fixed budgets turned from a stopping rule into a second dimension. Two aggregation choices carry the content: the mean covers only the seeds that **converged**, because including the others would require imputing a value and the only honest one ("did not converge") is not a number — the rate carries that half, and the two are read together; and NSGA-II gets **no field rather than a zero**, because "is the roster balanced?" is not a question one asks of a front, and a `0` there is a number someone would aggregate without noticing. Measured on the n = 20 battery: 20/20 seeds converge, at generation 34.8 ± 17.1, even though the out-of-stream confirmation rejects 71% of gate firings (50 of 70) — it delays convergence, it does not prevent it.
