...
**Two control arms at battery sample and budget** (n = 20, pop 300 × 150): the scalar GA with `LAMBDA_DRIFT = 0` (the counterfactual of the research question: balance without the identity term) and without the canonical seed. **Both were measured on the 2026-09-21 battery, and the λ = 0 arm is the strongest single result in it**: dropping the drift term **costs balance** (`dominance` 0.0534 against 0.0399 in medians, p_Holm = 0.038, medium effect) while destroying identity on all four rulers — drift 0.4089 against 0.2495, validator L1+2 5.7 against 11.5, Layer 3 0.90 against 2.45, τ +0.007 against +0.259, each at p_Holm ≤ 0.0045 with a large effect. τ = +0.007 is chance to three decimals. Hard counters is the only metric of the six that does not separate (p_Holm = 0.763). The unseeded arm separates on **drift only** (0.2759 against 0.2495, p_Holm = 0.043) and on nothing else, so initialization is not what the algorithm comparison is measuring. `multi_run` routes a run by what its body records: fully protocol → the battery paths; a **design**-only deviation (λ, dominance weights, selection, canonical seed, headline representative) with protocol sample and budget → `results/controls/` (citable); a **sample or budget** deviation (population, generations, number or start of seeds, sims) → `results/exploratory/`. `compare_algorithms --control` compares the battery with each control.
