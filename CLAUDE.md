...
- Weights 1.0 / 0.5 / 0.5 (max 2.0). **The secondaries are load-bearing, and that is measured**: with both at zero the GA reaches the best `global_term` of every sweep arm (0.0230) and still turns 8.6 of the 10 pairs into hard counters; `decis_term` reads 0.0000 on healthy final individuals because it works — removing it alone takes hard counters from 0.4 to 2.6 and `cap_term` from 0.0034 to 0.1188. Arms are compared by the terms and post-hoc metrics, never by the composite, which the weights define.
