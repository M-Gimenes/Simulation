...
| `MULTI_RUN_SIMS` | 200 | sims/matchup na reavaliação independente. **Independente de `SIMS_CONVERGENCE_CHECK`** apesar do mesmo valor: aquela roda *dentro* do laço a cada disparo do gate, esta uma vez por execução. Resolução suficiente para os agregados de n = 20 (o ruído é simétrico entre braços e a média o dilui), **insuficiente para ranquear rosters**: o desvio do `dominance` de um mesmo roster é 0,015–0,028, da ordem do valor evoluído (~0,04), e um veredito entre dois braços em 5 sementes inverteu contra a reavaliação a 1000 sims. Subir para 1000 custa 10.000 lutas por semente contra 67.500.000 da execução, mas obsoleta a bateria — pendência 5 de [10](10-known-issues.md) |
